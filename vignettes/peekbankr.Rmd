...
# The vignette reads live data from the peekbank dataset on Redivis, so it
# needs internet access and credentials. Chunks evaluate only when that is
# explicitly requested. They also carry purl = FALSE individually, because
# R CMD check tangles the vignette and runs the result, which ignores eval.
knitr::opts_chunk$set(eval = identical(Sys.getenv("PEEKBANK_NETWORK_TESTS"), "true"))
```
...
There are several different `get_` functions that you can use to extract different types of data from peekbank: 
...
**Technical note 1**: The first request will open a browser window to authorize Redivis access (or set a `REDIVIS_API_TOKEN`).
